...
static int kscan_matrix_interrupt_configure(const struct device *dev, const gpio_flags_t flags) {
    ...
    for (int i = 0; i < data->inputs.len; i++) {
        //这里的inputs 都是行,行是pulldown的,如果变成1就会唤醒 高电平中断 GPIO_INT_LEVEL_HIGH 等价于
...
static int kscan_matrix_read(const struct device *dev) {
 ...
 extern bool zmk_usb_is_hid_ready(void);
...
//  extern enum zmk_transport zmk_endpoint_get_preferred_transport(void) ;
    if (qf_cnt % 5000 == 0  ) {
        ...
        //LOG_INF("config -> rows %d cols %d",config->rows, config->cols);
        // LOG_INF("usb ready = %d", zmk_usb_is_hid_ready());
        LOG_INF("ble ready = %d", zmk_ble_active_profile_is_connected()); 
        #endif  
    }
    ...
    // bool usb_is_on = zmk_usb_is_hid_ready();
    if (continue_scan) {
...
static void kscan_matrix_work_handler(struct k_work *work) {
    ...
    //  LOG_INF(". 22222 kscan_matrix_work_handler");
...
static int kscan_matrix_init_input_inst(const struct device *dev, const struct kscan_gpio *gpio) {
    ...
    if (err) {
        LOG_ERR("77777 Unable to configure pin %u on %s for input", gpio->spec.pin,
                gpio->spec.port->name);
        return err;
    }
...
static int kscan_matrix_init_output_inst(const struct device *dev,
    ...
    if (err) {
        LOG_ERR("8888 Unable to configure pin %u on %s for output", gpio->pin, gpio->port->name);
        return err;
    }
